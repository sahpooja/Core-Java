...
import com.eb.h2.service.JobService;
...
public class JobController 
{
	...
	@PostMapping("/jobs")
	public ResponseEntity<Job> saveJob(@RequestBody Job 
	job){
	 return new 
	...
	job, @PathVariable("id") long id) throws 
	ResourceNotFoundException{
	 job.setId(id);
	 return new 
	ResponseEntity<Job>(jobService.updateJob(job),HttpStatus.
	CREATED);
	}
	@DeleteMapping("/jobs/{id}")
	public ResponseEntity<String> 
	deleteJobs(@PathVariable("id") long id) throws 
	ResourceNotFoundException{
